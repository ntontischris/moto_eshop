...
describe("product rail", () => {
  it("renders a Nour-inspired lightweight product gallery", () => {
    expect(railSource).toContain("const galleryProducts = products.slice(0, 5);");
  ...
  it("styles the gallery with calm image plates and no destructive image effects", () => {
    expect(componentsCss).toContain("/* === nour inspired product gallery === */");
    ...
    expect(componentsCss).not.toContain(".v3-rail--nour-gallery .v3-gallery-plate img {\n  mix-blend-mode");
  });
});
